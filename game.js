...
function initBattle() {
  ...
  // 无尽:声誉每三波回满。没有终点的跑法里,一次性的 8 点等于招牌卡用完就废
  if (G.endless && G.battleIdx % 3 === 0) G.rep = REP_MAX;
...
function waveBonus() {
  const w = (B && B.enemy && B.enemy.wave) ? B.enemy.wave : 0;
  return Math.floor(w / 3);   // 每三波 +1,不是每波
}
...
function grossIncome() { return 3 + B.permIncome + waveIncome(); }
...
/* 这一场是不是 Boss。介绍页和战斗初始化必须用同一份判定 */
function isBossBattle() {
  return G.endless ? (G.battleIdx % 4 === 3) : (G.battleIdx === 2 || G.hell);
}

function showBattleIntro() {
  rerender = showBattleIntro;
  // 无尽的对手队列是按需续的:介绍页比 initBattle 先跑,这里不续就会读到 undefined
  while (G.endless && G.battleIdx >= G.queue.length) queueMoreOpponents();
  const id = G.queue[G.battleIdx];
  const boss = isBossBattle();
